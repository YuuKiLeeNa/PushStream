...
#include<functional>
#include "rnnoiseWarp/rnnoiseHelp.h"
...
class CSource:public CWasapi<CSource/*<CHANNELS>*/>, public CRingBuffer/*<CHANNELS>*/, public AudioPeakMagnitudeCalc,public rnnoiseHelp,public CSourceType
{
public:
	...
	CSource(const std::string& name, obs_peak_meter_type peak_calc_type, AUDIO_DATA_CALC_CALL_BACK audio_call_back, typename CWasapi<CSource>::SourceType type) :source_name(name)
		...
		, CSourceType(CSourceType::SourceDefineType::ST_AUDIO)
	...
	virtual ~CSource() 
	{
		...
		//audio thread may block in WaitForMultipleObjects status when thread stop.So tun on dataLengthNoticeSignal
	...
	void setAudioFilterStatus(bool b) { m_bEnableAudioFilter = b; }
	bool getAudioFilterStatus() { return m_bEnableAudioFilter; }
protected:
...
protected:
	...
	std::atomic_bool m_bEnableAudioFilter = false;
};
...
bool CSource::process_audio(const struct obs_source_audio* audio, struct obs_source_audio* audio_out)
{
		...
		m_in_channel_layout != audio->speakers)
	{
		...
		if (!rnnoiseHelp::setAudioInfo(m_in_channel_layout
			, m_in_format
			, m_in_sample_rate
		...
		{
			PrintE("init rnnoiseHelp::setAudioInfo error");
			return false;
		}
		if (!rnnoiseHelp::setOnRnnModel()) 
		{
			PrintE("init rnnoiseHelp::setOnRnnModel error");
			return false;
		}
	}
	...
	if (m_bEnableAudioFilter)
	{
		UPTR_FME fme = rnnoiseHelp::translate((uint8_t**)audio->data, (int)audio->frames, &resample_offset);
		if (fme && fme->nb_samples > 0) 
		{
			av_samples_alloc(audio_out->data, NULL, fme->ch_layout.nb_channels, fme->nb_samples,(AVSampleFormat)fme->format, 16);
			int bytes = av_get_bytes_per_sample((AVSampleFormat)fme->format);
			for (int ch = 0; ch < fme->ch_layout.nb_channels; ++ch)
				memcpy(audio_out->data[ch], fme->data[ch], bytes * fme->nb_samples);
			audio_out->timestamp = audio->timestamp;
			audio_out->frames = fme->nb_samples;
		}
		else 
		{
			audio_out->frames = 0;
		...
		av_samples_alloc(audio_out->data, NULL, chlay.nb_channels, audio->frames, m_out_format, 16);
