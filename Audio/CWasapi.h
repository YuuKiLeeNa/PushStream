...
public:
	...
	CWasapi(CWasapi::SourceType type);
	CWasapi() :CWasapi(CWasapi::SourceType::DeviceOutput) {}
	~CWasapi();
	bool init();
	static std::vector<CWasapi::AudioDeviceInfo>getMicrophoneDevices();
	static std::vector<CWasapi::AudioDeviceInfo>getDesktopAudioDevices();
	static bool getAudioDevices(EDataFlow device_type, std::vector<CWasapi::AudioDeviceInfo>& devices);
	void Start(){SetEvent(initSignal);}
	void Stop();
	void OnRestart(){SetEvent(receiveSignal);}
	void Activate();
	void Deactivate();
	void SetDefaultDevice(EDataFlow flow, ERole role, LPCWSTR id);
	inline std::string DeviceName() { return device_name; }
	inline speaker_layout GetSpeakerLayout() {return speakers;}
	inline int GetSpeakerLayoutCh() {return get_audio_channels(speakers);}
	//WinHandle getDataLengthNoticeSignal() { return dataLengthNoticeSignal; }
	int isExit();
	void setUseDefaultDevice() { isDefaultDevice = true; OnRestart(); }
	void setUseSpecifyDeviceId(const std::string &deviceid) {
...
protected:
	...
	ComPtr<IMMDevice> InitDevice(IMMDeviceEnumerator* enumerator, bool isDefaultDevice, SourceType type, const std::string& device_id);
	static std::string GetDeviceName(IMMDevice* device);
	ComPtr<IAudioClient> InitClient(
...
protected:
	...
	//int data_len_size_notice = 2048;



	SourceType sourceType;// = SourceType::DeviceOutput;
	bool previouslyFailed = false;

	std::atomic<bool> useDeviceTiming = false;
	std::atomic<bool> isDefaultDevice = true;
...
template<typename SOURCE>
CWasapi<SOURCE>::CWasapi(CWasapi::SourceType type) :sourceType(type)
{
	useDeviceTiming = sourceType == CWasapi::SourceType::DeviceOutput ? true: false;
...
template<typename SOURCE>
CWasapi<SOURCE>::~CWasapi()
{
	Stop();
}

template<typename SOURCE>
bool CWasapi<SOURCE>::init()
{
	bool b = true;
...
template<typename SOURCE>
std::vector<typename CWasapi<SOURCE>::AudioDeviceInfo> CWasapi<SOURCE>::getMicrophoneDevices()
{
...
template<typename SOURCE>
std::vector<typename CWasapi<SOURCE>::AudioDeviceInfo> CWasapi<SOURCE>::getDesktopAudioDevices()
{
	std::vector<CWasapi::AudioDeviceInfo>devices;
...
template<typename SOURCE>
bool CWasapi<SOURCE>::getAudioDevices(EDataFlow type, std::vector<CWasapi::AudioDeviceInfo>& devices)
{
	bool b = true;
	WinModule mmdevapi_module_tmp;
	mmdevapi_module_tmp = LoadLibrary(L"Mmdevapi");
	if (!mmdevapi_module_tmp.Valid())
	{
		PrintE("load dll error for Mmdevapi");
		b = false;
		//return false;
	...
	devices.clear();
	ComPtr<IMMDeviceEnumerator>enumerator_tmp;

	HRESULT hr = CoCreateInstance(__uuidof(MMDeviceEnumerator), nullptr,
		CLSCTX_ALL,
		IID_PPV_ARGS(enumerator_tmp.Assign()));
	if (FAILED(hr))
	{
		PrintE("Failed to create enumerator", hr);
		return false;
	...
	if (enumerator_tmp)
	{
		HRESULT res = enumerator_tmp->EnumAudioEndpoints(type, DEVICE_STATE_ACTIVE, collection.Assign());
		if (FAILED(res))
		{
...
template<typename SOURCE>
void CWasapi<SOURCE>::Stop()
{
	SetEvent(stopSignal);

	//blog(LOG_INFO, "WASAPI: Device '%s' Terminated", device_name.c_str());
	PrintD("WASAPI: Device '%s' Terminated", device_name.c_str());
...
template<typename SOURCE>
void CWasapi<SOURCE>::Activate()
{
	if (!reconnectThread.Valid()) {
		ResetEvent(reconnectExitSignal);
		reconnectThread = CreateThread(nullptr, 0,
			CWasapi<SOURCE>::ReconnectThread,
			this, 0, nullptr);
	}
}

template<typename SOURCE>
void CWasapi<SOURCE>::Deactivate()
{
	if (reconnectThread.Valid()) {
		SetEvent(reconnectExitSignal);
		WaitForSingleObject(reconnectThread, INFINITE);
...
template<typename SOURCE>
void CWasapi<SOURCE>::SetDefaultDevice(EDataFlow flow, ERole role, LPCWSTR id)
{
	if (!isDefaultDevice)
		return;
	...
	PrintD("WASAPI: Default %s device changed",input ? "input" : "output");
	SetEvent(restartSignal);
}

template<typename SOURCE>
int CWasapi<SOURCE>::isExit() 
{
	int v = 0;
	DWORD ret = WaitForSingleObject(exitSignal, 0);
	switch (ret)
	{
	case WAIT_OBJECT_0:
		v = 1;
...
template<typename SOURCE>
DWORD WINAPI CWasapi<SOURCE>::CaptureThread(LPVOID param)
{
	//s_thread_index
...
template<typename SOURCE>
ComPtr<IMMDevice> CWasapi<SOURCE>::InitDevice(IMMDeviceEnumerator* enumerator, bool isDefaultDevice,typename CWasapi<SOURCE>::SourceType type, const std::string& device_id)
{
	ComPtr<IMMDevice> device;
	if (isDefaultDevice) {
...
template<typename SOURCE>
std::string CWasapi<SOURCE>::GetDeviceName(IMMDevice* device)
{
	std::string device_name;
	ComPtr<IPropertyStore> store;
	HRESULT res;
...
template<typename SOURCE>
ComPtr<IAudioClient> CWasapi<SOURCE>::InitClient(
	IMMDevice* device,typename CWasapi<SOURCE>::SourceType type, DWORD process_id,
	PFN_ActivateAudioInterfaceAsync activate_audio_interface_async,
	speaker_layout& speakers, audio_format& format,
	uint32_t& samples_per_sec)
{
...
template<typename SOURCE>
void CWasapi<SOURCE>::InitFormat(const WAVEFORMATEX* wfex,
	enum speaker_layout& speakers,
	enum audio_format& format, uint32_t& sampleRate)
{
	DWORD layout = 0;
...
template<typename SOURCE>
speaker_layout CWasapi<SOURCE>::ConvertSpeakerLayout(DWORD layout, WORD channels)
{
	switch (layout) {
...
template<typename SOURCE>
ComPtr<IAudioCaptureClient> CWasapi<SOURCE>::InitCapture(IAudioClient* client,
...
template<typename SOURCE>
void CWasapi<SOURCE>::ClearBuffer(IMMDevice* device)
{
	CoTaskMemPtr<WAVEFORMATEX> wfex;
	HRESULT res;
	LPBYTE buffer;
...
template<typename SOURCE>
bool CWasapi<SOURCE>::Initialize()
{
	ComPtr<IMMDevice> device;
...
template<typename SOURCE>
bool CWasapi<SOURCE>::TryInitialize()
{
...
template<typename SOURCE>
bool CWasapi<SOURCE>::ProcessCaptureData()
{
	HRESULT res;
	LPBYTE buffer;
	UINT32 frames;
	...
	while (true) {
		...
		{
			PrintE("%s:[WASAPISource::ProcessCaptureData] doesn't realize", device_name.c_str());
			return false;
		}
		...
		if (FAILED(res)) {
			if (res != AUDCLNT_E_DEVICE_INVALIDATED)
				PrintW("%s:[WASAPISource::ProcessCaptureData]"
					" capture->GetNextPacketSize"
					" failed: %lX",
					device_name.c_str(),res);
			return false;
		}
		...
		if (!sawBadTimestamp &&
			...
			PrintW("%s:[WASAPISource::ProcessCaptureData]"
				" Timestamp error!", device_name.c_str());
			sawBadTimestamp = true;
		}

		if (flags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) {
			/* libobs should handle discontinuities fine. */
			PrintD("%s:[WASAPISource::ProcessCaptureData]"
				" Discontinuity flag is set.", device_name.c_str());
		}

		if (flags & AUDCLNT_BUFFERFLAGS_SILENT) {
			PrintD("%s:[WASAPISource::ProcessCaptureData]"
				" Silent flag is set.", device_name.c_str());
		...
		/*if (reroute_target) {
			...
			((SOURCE*)(this))->source_output_audio(&data);
		//}
