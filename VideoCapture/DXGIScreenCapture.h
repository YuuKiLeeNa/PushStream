...
{
public:
	DXGIScreenCapture(CaptureEvents* events);
	...
	void setCaptureFrameCallBack(const std::function<void(UPTR_FME)>&fun);
...
private:
	...
	std::unique_ptr<std::function<void(UPTR_FME)>>m_captureFrameCallBack;
	imageConvert m_con;
};
