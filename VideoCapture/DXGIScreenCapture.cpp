...
DXGIScreenCapture::DXGIScreenCapture(CaptureEvents* events)
	: VideoCapture(events)
...
bool DXGIScreenCapture::Init(int display_index)
{
	...
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to create d3d11 device.");
		return false;
	}
	...
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to create dxgi factory.");
		Destroy();
		return false;
	}
	...
	if (dxgi_adapter.Get() == nullptr) {
		PrintE("[DXGIScreenCapture] DXGI adapter not found.");
		Destroy();
		return false;
	}

	if (dxgi_output.Get() == nullptr) {
		PrintE("[DXGIScreenCapture] DXGI output not found.");
		Destroy();
		return false;
	}
	...
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to query interface dxgiOutput1.");
		Destroy();
		return false;
	}
	...
	if (FAILED(hr)) {
		...
		PrintE("[DXGIScreenCapture] Failed to get duplicate output.");
		Destroy();
		return false;
	}
...
int DXGIScreenCapture::CreateSharedTexture()
{
	...
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to create texture.\n");
		return -1;
	}
	...
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to query IDXGIResource interface from texture.\n");
		return -1;
	}

	hr = dxgiResource->GetSharedHandle(&texture_handle_);
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to get shared handle.\n");
		return -1;
	}

	hr = shared_texture_->QueryInterface(_uuidof(IDXGIKeyedMutex), reinterpret_cast<void **>(keyed_mutex_.GetAddressOf()));
	if (FAILED(hr)) {
		PrintE("[DXGIScreenCapture] Failed to create key mutex.\n");
		return -1;
	}
...
int DXGIScreenCapture::AquireFrame()
{
	...
	if (FAILED(hr)) {
		if (hr == DXGI_ERROR_WAIT_TIMEOUT) {
			//PrintD("Microsoft::WRL::ComPtr<IDXGIOutputDuplication>::AcquireNextFrame error:DXGI_ERROR_WAIT_TIMEOUT");
			return -1;
		}
		if (hr == DXGI_ERROR_INVALID_CALL) {
			PrintE("Microsoft::WRL::ComPtr<IDXGIOutputDuplication>::AcquireNextFrame error:DXGI_ERROR_INVALID_CALL .Now restart");
			m_callBack->RestartVideo(0);
			return -1;
		}
		if (hr == DXGI_ERROR_ACCESS_LOST) {
			PrintD("Microsoft::WRL::ComPtr<IDXGIOutputDuplication>::AcquireNextFrame error:DXGI_ERROR_INVALID_CALL");
			return -1;
		}
		PrintE("Microsoft::WRL::ComPtr<IDXGIOutputDuplication>::AcquireNextFrame error:%i", hr);
		return -1;
	}
	...
	if (FAILED(hr)) {
		PrintD("Microsoft::WRL::ComPtr<ID3D11Texture2D>::QueryInterface error:%i", hr);
		return -1;
	}
	...
	if (FAILED(hr)) {
		PrintD("Microsoft::WRL::ComPtr<IDXGISurface1>::QueryInterface error:%i", hr);
		return -1;
	}
				...
				if (m_con(in_data, in_line, tmp_->data, tmp_->linesize))
				{
					...
					tmp = av_frame_clone(yuv420p_);
					{
						...
						if (m_captureFrameCallBack)
							(*m_captureFrameCallBack)(UPTR_FME(tmp));
					}
				}
			}
		}
...
void DXGIScreenCapture::setCaptureFrameCallBack(const std::function<void(UPTR_FME)>&fun)
{
	std::lock_guard<std::mutex>lock(mutex_fun_);
