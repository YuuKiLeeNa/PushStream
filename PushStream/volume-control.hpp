...
private:
	...
	PSSource m_source;
public:
	explicit VolumeMeter(QWidget *parent,
...
private slots:
	...
	void slotAudioData(QVector<QVector<float>>sets);

signals:
	void ConfigClicked();
	void signalAudioData(QVector<QVector<float>>);
	void signalSendDB(float);
public:
	explicit VolControl(PSSource source, bool showConfig = false,
			    bool vertical = false);
	~VolControl();

	inline PSSource GetSource() const { return source; }
	...
	void Audio_Data_Call_back(float(*magnitude)[MAX_AUDIO_CHANNELS], float(*peak)[MAX_AUDIO_CHANNELS]);
};
