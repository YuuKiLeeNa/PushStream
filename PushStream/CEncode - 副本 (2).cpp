...
bool CEncode::init()
{
	...
	if (avformat_alloc_output_context2(&fmtCtx, nullptr, nullptr, m_file.c_str()) < 0)
		return false;
	...
	auto fun = [this,&fmtCtx](std::pair<const AVCodec*, const AVCodecHWConfig*> CEncode::* pCodecConf, AVCodecID idDefault, AVPixelFormat accptPix, AVHWDeviceType type)
		{
			if (!(this->*pCodecConf).first)
			{
				AVCodecID id;
				if ((!fmtCtx->oformat) || fmtCtx->oformat->video_codec == AV_CODEC_ID_NONE)
					id = idDefault;
				else
				{
					if (&(this->*pCodecConf) == &m_videoCodecConfig)
						id = fmtCtx->oformat->video_codec;
					else
						assert(0);
				}
	...
	if ((m_flags & ENCODE_VIDEO) && fmtCtx->oformat->video_codec != AV_CODEC_ID_NONE)
	{
		...
		{
			fun(&CEncode::m_videoCodecConfig, fmtCtx->oformat->video_codec, m_decodeHwFmt, m_decodeAVHWDeviceType);
		...
		if (!m_codecVideo)
		{
			m_codecVideo = avcodec_find_encoder(fmtCtx->oformat->video_codec);
			if (!m_codecVideo)
				return false;
		}
		...
		if (codecCtxVideo)
		{
			...
			codecCtxVideo->time_base = m_ratTimeBase;
			...
			if (m_videoCodecConfig.second)
			{
				if (m_pAVBufferRef != nullptr
					...
					&& m_decodeHwFmt != AV_PIX_FMT_NONE)
				{
					codecCtxVideo->hw_frames_ctx = av_buffer_ref(m_pAVBufferRef);
					codecCtxVideo->pix_fmt = m_decodeHwFmt;//((AVHWFramesContext*)m_decodecCtxVideo->hw_frames_ctx->data)->format;
				}
				else
				{
					codecCtxVideo->pix_fmt = m_videoCodecConfig.second->pix_fmt;
			...
			{
				if (m_videoStream = avformat_new_stream(fmtCtx, m_codecVideo))
					if (avcodec_parameters_from_context(m_videoStream->codecpar, codecCtxVideo) >= 0)
					{
						m_videoStream->time_base = { 1, AV_TIME_BASE };
						m_videoStream->id = fmtCtx->nb_streams - 1;
	...
	if ((m_flags & ENCODE_AUDIO) && fmtCtx->oformat->audio_codec)
	{
		if (!m_codecAudio)
		{
			m_codecAudio = avcodec_find_encoder(fmtCtx->oformat->audio_codec);
			if (!m_codecAudio)
		...
		if (codecCtxAudio)
		{
			...
			initSuitableAudioCodec(codecCtxAudio);
			if (fmtCtx->oformat->flags & AVFMT_GLOBALHEADER)
			...
			if ((ret=avcodec_open2(codecCtxAudio, m_codecAudio, nullptr)) == 0)
			{
				...
				if (m_audioStream = avformat_new_stream(fmtCtx, m_codecAudio))
				{
					if (avcodec_parameters_from_context(m_audioStream->codecpar, codecCtxAudio) >= 0)
					{
						m_audioStream->time_base = { 1, codecCtxAudio->sample_rate };
						m_audioStream->id = fmtCtx->nb_streams - 1;
...
void CEncode::initSuitableAudioCodec(AVCodecContext*codec_ctx)
{
	if (!m_codecAudio)
		return;
