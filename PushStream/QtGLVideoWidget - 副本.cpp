...
QtGLVideoWidget::QtGLVideoWidget(QWidget* pParentWidget) :QOpenGLWidget(pParentWidget)
{
	...
	//m_tex_value = std::atomic_fetch_add(&vert_index, 3);
}
...
//std::atomic_int QtGLVideoWidget::vert_index = 0;
...
void QtGLVideoWidget::initializeGL()
{
	...
	m_program.link();
	m_program.setUniformValue("y_tex", /*m_tex_value + */0);
	m_program.setUniformValue("u_tex", /*m_tex_value + */1);
	m_program.setUniformValue("v_tex", /*m_tex_value + */2);
...
void QtGLVideoWidget::paintGL()
{
	...
	*/
	...
	glActiveTexture(GL_TEXTURE0/* + m_tex_value*/);
	glBindTexture(GL_TEXTURE_2D, y_tex);
	if (bUploadTex)
		setYPixels(m_frame_tmp->data[0], m_frame_tmp->linesize[0]);
	glActiveTexture(GL_TEXTURE1/* + m_tex_value*/);
	glBindTexture(GL_TEXTURE_2D, u_tex);
	if (bUploadTex)
		setUPixels(m_frame_tmp->data[1], m_frame_tmp->linesize[1]);
	glActiveTexture(GL_TEXTURE2/* + m_tex_value*/);
	glBindTexture(GL_TEXTURE_2D, v_tex);
	if (bUploadTex)
