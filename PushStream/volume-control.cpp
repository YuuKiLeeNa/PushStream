...
void VolControl::VolumeMuted(bool muted)
{
	...
	auto newState = Qt::CheckState::Checked;//GetCheckState(muted, unassigned);
...
{
	bool muted = false;//obs_source_muted(source);
	//bool unassigned = IsSourceUnassigned(source);
...
void VolControl::SetMuted(bool)
{
	...
	bool unassigned = false;//IsSourceUnassigned(source);
...
void VolControl::SliderChanged(int vol)
{
	...
	float db = log_def_to_db(float(vol) / FADER_PRECISION);

	updateText(db);

	emit signalSendDB(db);
...
void VolControl::updateText(float db)
{
	QString text;
	//float db = 0;//obs_fader_get_db(obs_fader);
	...
	QString sourceName = QString::fromStdString(source->DeviceName());//obs_source_get_name(source);
	QString accText = QTStr(accTextLookup).arg(sourceName);

	slider->setAccessibleName(accText);
}

void VolControl::slotAudioData(QVector<QVector<float>> sets)
{


	float db = log_def_to_db(float(slider->value()) / FADER_PRECISION);


	float mul = mute->isChecked() ? 0.0f:db_to_mul(db);//muted && !obs_source_muted(source) ? 0.0f : db_to_mul(volmeter->cur_db);

	float magnitude[MAX_AUDIO_CHANNELS], peak[MAX_AUDIO_CHANNELS];
	float(*arr[2])[MAX_AUDIO_CHANNELS] = {&magnitude,  &peak};
	auto iter_arr = std::begin(arr);

	for (auto iter = sets.cbegin(); iter != sets.cend(); ++iter,++iter_arr)
	{
		auto iterChild = iter->cbegin(); 
		auto iter_arr_child = std::begin(**iter_arr); 
		for (; iterChild != iter->cend(); ++iterChild, ++iter_arr_child)
			*iter_arr_child = *iterChild;
	}


	float r_magnitude[MAX_AUDIO_CHANNELS] = {0.0f}, r_peak[MAX_AUDIO_CHANNELS] = {0.0f}, r_inputPeak[MAX_AUDIO_CHANNELS] = {0.f};
	for (int channel_nr = 0; channel_nr < MAX_AUDIO_CHANNELS;
		channel_nr++) {
		r_magnitude[channel_nr] = mul_to_db(magnitude[channel_nr] * mul);
		r_peak[channel_nr] = mul_to_db(peak[channel_nr] * mul);

		/* The input-peak is NOT adjusted with volume, so that the user
		 * can check the input-gain. */
		r_inputPeak[channel_nr] = mul_to_db(peak[channel_nr]);
	}
	volMeter->setLevels(r_magnitude, r_peak, r_inputPeak);
}
...
void VolControl::initConnect()
{
	QObject::connect(this, SIGNAL(signalAudioData(QVector<QVector<float>>)), this, SLOT(slotAudioData(QVector<QVector<float>>)));
}
...
VolControl::VolControl(PSSource audioSource, bool showConfig, bool vertical)
	  ...
	  contextMenu(nullptr)
{
	...
	QString sourceName = QString::fromStdString(source->getSourceName());//obs_source_get_name(source);
	...
	if (vertical) {
		...
		volMeter = new VolumeMeter(nullptr, source,/*obs_volmeter,*/ true);
		...
		volMeter = new VolumeMeter(nullptr, source,/*obs_volmeter,*/ false);
	...
	slider->setValue(int(FADER_PRECISION));
	...
	initConnect();
	SliderChanged(int(FADER_PRECISION));
...
void VolControl::Audio_Data_Call_back(float(*magnitude)[MAX_AUDIO_CHANNELS], float(*peak)[MAX_AUDIO_CHANNELS])
{
	QVector<QVector<float>>sets;
	sets.reserve(2);
	QVector<float>magnitude_set(std::begin(*magnitude), std::end(*magnitude));
	QVector<float>peak_set(std::begin(*magnitude), std::end(*magnitude));
	sets.push_back(std::move(magnitude_set));
	sets.push_back(std::move(peak_set));
	emit signalAudioData(std::move(sets));
}
...
VolumeMeter::VolumeMeter(QWidget *parent, PSSource ptr,/*obs_volmeter_t* obs_volmeter,*/
			 bool vertical)
	: QWidget(parent),
		m_source(ptr),
	  ...
	  vertical(vertical)
{
	...
	channels = m_source->GetSpeakerLayoutCh();//PSSource::element_type::s_channels;//decltype(source)//(int)audio_output_get_channels(obs_get_audio());
...
{
	int currentNrAudioChannels = m_source->GetSpeakerLayoutCh();//PSSource::element_type::s_channels;// obs_volmeter_get_nr_channels(obs_volmeter);
