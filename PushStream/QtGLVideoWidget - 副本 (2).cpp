...
void QtGLVideoWidget::calculate_display_rect(AVFrame* f)
{
	if(f)
		calculate_display_rect(0, 0, width(), height(), f->width, f->height, f->sample_aspect_ratio);
}

void QtGLVideoWidget::calculate_display_rect(
...
void QtGLVideoWidget::paintGL()
{
	...
	if (!m_isOpenGLInit)
	{
		...
		emit signalsGetFrame();
		return;
		//setYPixels(m_frameSave.frame->data[0], m_frameSave.frame->linesize[0]);
		//setUPixels(m_frameSave.frame->data[1], m_frameSave.frame->linesize[1]);
		//setVPixels(m_frameSave.frame->data[2], m_frameSave.frame->linesize[2]);
	}
