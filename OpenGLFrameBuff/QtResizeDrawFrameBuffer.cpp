...
void QtResizeDrawFrameBuffer::initializeGL()
{
	...
	//m_threadDrawFrameBuffer.reset();
	//m_bStopThreadDrawFrameBuffer = false;
	//m_threadDrawFrameBuffer = std::shared_ptr<std::thread>(new std::thread([this]() {drawFrameBufferThreadFun(); }), [this](std::thread* t) 
	//	{
	//		if (t)
	//		{
	//			m_bStopThreadDrawFrameBuffer = true;
	//			m_cond.notify_all();
	//			if (t->joinable())
	//				t->join();
	//			delete t;
...
void QtResizeDrawFrameBuffer::createFramebufferObject()
{
	if (m_fbo) 
		delete m_fbo;
...
void QtResizeDrawFrameBuffer::drawFrameBuffer()
{
	...
	if (m_fbo->width() != m_iPreFrameWidth || m_fbo->height() != m_iPreFrameHeight) {
		createFramebufferObject();
...
void QtResizeDrawFrameBuffer::writeRGBImage()
{
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE)
	{
		...
		cv::Mat mat(cv::Size(wfbo, hfbo), CV_8UC3), dst;
		glPixelStorei(GL_PACK_ROW_LENGTH, wfbo);
...
//void QtResizeDrawFrameBuffer::drawFrameBufferThreadFun()
//{
//	if (!m_ContextWorkThread)
//	{
//		m_ContextWorkThread = new std::remove_pointer_t<decltype(m_ContextWorkThread)>;
//		m_ContextWorkThread->setFormat(format());
//		m_ContextWorkThread->setShareContext(context());
//		m_ContextWorkThread->create();
//	}
//
//	if (!m_programDrawText) {
//		m_programDrawText = new QOpenGLShaderProgram;
...
//
//	int err;
//	while (!m_bStopThreadDrawFrameBuffer)
//	{
//		{
//			std::unique_lock<std::mutex>mux(m_mutex);
//			m_cond.wait(mux, [this]()
//				{
//					return m_bUploadTex | m_bStopThreadDrawFrameBuffer;
//				});
//
//			if (m_bStopThreadDrawFrameBuffer)
//				break;
//		}
//
//		QOffscreenSurface surface;
//		surface.create();
//		if(m_ContextWorkThread->makeCurrent(&surface))
//		{
//			initializeOpenGLFunctions();
//			std::lock_guard<std::mutex>lock(m_mutex_fbo);
//			drawFrame();
//			m_ContextWorkThread->doneCurrent();
//		}
//		update();
//	}
//
...
//	m_programDrawText->destroyed();
//	delete m_programDrawText;
//	m_programDrawText = nullptr;
//	QtGLVideoWidget::deleteObject();
//
//
//	m_ContextWorkThread->destroyed();
//	delete m_ContextWorkThread;
//	m_ContextWorkThread = nullptr;
//}
