...
void QtResizeDrawFrameBuffer::renderFrame(AVFrame* f)
{
	ResizeGLWidget::setFrame(f);
	QtGLVideoWidget::setFrame(f);
	makeCurrent();
	{
...
std::shared_ptr<CalcGLDraw> QtResizeDrawFrameBuffer::getMouseRayGLDrawText(int width, int height, double xpos, double ypos, glm::mat4 view, glm::mat4 projection) {
	...
	for (int i = isize - 1; i >=0; --i)
	{
		...
		QRectF rect(m_vecDrawComponent[i]->m_xCol, m_vecDrawComponent[i]->m_yRow, m_vecDrawComponent[i]->m_wImg, m_vecDrawComponent[i]->m_hImg);
