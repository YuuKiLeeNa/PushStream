...
int DXGIScreenCapture::AquireFrame()
{
	...
	if (!FAILED(hr)) {
		if (dsec.pData != NULL) {
			...
			{
				if (m_con(in_data, in_line, out_data, out_line)) 
				{
					if (m_captureFrameCallBack)
						(*m_captureFrameCallBack)(image_ptr_, image_width, image_height);
				}
			}
		}
...
bool DXGIScreenCapture::CaptureFrame(std::shared_ptr<uint8_t>& bgra_image, uint32_t& width, uint32_t& height)
{
	std::lock_guard<std::mutex> locker(mutex_);
...
void DXGIScreenCapture::setCaptureFrameCallBack(const std::function<void(std::shared_ptr<uint8_t>& bgra_image,const uint32_t&,const uint32_t&)>& fun)
{
