...
int CEncode::initAudioCodecCtxStream(AVFormatContext*& ctx)
{
	...
	if (codecCtxAudio)
	{
		...
		if (!initSuitableAudioCodec(codecCtxAudio))
				return -2;
		codecCtxAudio->bit_rate = m_option.av_opt_get_int("b:a", 128000);
		if (ctx->oformat->flags & AVFMT_GLOBALHEADER)
...
bool CEncode::init()
{
	...
	if (m_flags & ENCODE_VIDEO)
	{
		...
		std::vector<std::pair<const AVCodec*, const AVCodecHWConfig*>>hw_sets = CSelCodec::getHWEncodeCodecByStreamIdAndAcceptPixFmt(id, AV_PIX_FMT_NONE, AV_HWDEVICE_TYPE_NONE);
		const AVCodec* codec = avcodec_find_encoder(id);
		if (codec)
			hw_sets.push_back({ codec , NULL });
			//hw_sets.insert(hw_sets.begin(), {codec , NULL});
