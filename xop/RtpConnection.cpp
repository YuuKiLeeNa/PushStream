...
RtpConnection::RtpConnection(std::weak_ptr</*TcpConnection*/toolkit::TcpClient> rtsp_connection)
    : rtsp_connection_(rtsp_connection)
{
	...
	rtsp_ip_ = conn->get_peer_ip();
	rtsp_port_ = conn->get_peer_port();
}
...
RtpConnection::~RtpConnection()
{
	for(int chn=0; chn<MAX_MEDIA_CHANNEL; chn++) {
		...
		if (rtpfd_[chn]) {
			rtpfd_[chn]->closeSock();
		}

		/*if(rtcpfd_[chn] > 0) {
			SocketUtil::Close(rtcpfd_[chn]);*/
		if (rtcpfd_[chn]) {
			rtcpfd_[chn]->closeSock();
		}
	}
}

/*int*/std::thread::id RtpConnection::GetId() const
{
	auto conn = rtsp_connection_.lock();
	if (!conn) {
		return std::thread::id();//-1;
	}
	RtspConnection *rtspConn = (RtspConnection *)conn.get();
	return rtspConn->getPoller()->getThreadId();//GetId();
}
...
bool RtpConnection::SetupRtpOverTcp(MediaChannelId channel_id, uint16_t rtp_channel, uint16_t rtcp_channel)
{
	...
	rtpfd_[channel_id] = conn->getSock();
	rtcpfd_[channel_id] = conn->getSock();
...
bool RtpConnection::SetupRtpOverUdp(MediaChannelId channel_id, uint16_t rtp_port, uint16_t rtcp_port)
{
	...
	for (int n = 0; n <= 10; n++) {
		...
		rtpfd_[channel_id] = toolkit::Socket::createSocket(conn->getPoller());
		if (!rtpfd_[channel_id]->bindUdpSock(local_rtp_port_[channel_id])) 
		{
			rtpfd_[channel_id]->closeSock();
			continue;
		}
		rtcpfd_[channel_id] = toolkit::Socket::createSocket(conn->getPoller());
		if (!rtcpfd_[channel_id]->bindUdpSock(local_rtcp_port_[channel_id]))
		{
			rtcpfd_[channel_id]->closeSock();
			continue;
		}
	...
	std::weak_ptr<std::remove_reference_t<decltype(rtpfd_[channel_id])>::element_type>weak_ptrRtp = rtpfd_[channel_id];
	rtpfd_[channel_id]->getPoller()->async([weak_ptrRtp] {
		if (auto strong_ptrRtp = weak_ptrRtp.lock()) {
			toolkit::SockUtil::setSendBuf(strong_ptrRtp->rawFD(), 50 * 1024);
		}
		});
...
int RtpConnection::SendRtpOverUdp(MediaChannelId channel_id, RtpPacket pkt)
{
	...
	auto ret = rtpfd_[channel_id]->send((const char*)pkt.data.get() + 4, pkt.size - 4);
