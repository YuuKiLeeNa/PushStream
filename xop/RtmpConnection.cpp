...
void RtmpConnection::onConnect(const toolkit::SockException& ex) {
	...
	{
		auto rtmpClient = rtmp_client_.lock();
		if (rtmpClient)
		{
			setPlayCB(rtmpClient->FrameCB());
		}
		this->Handshake();

		std::shared_ptr<RtmpConnection>shared_ptr = std::dynamic_pointer_cast<RtmpConnection>(shared_from_this());
		std::weak_ptr<RtmpConnection>weak_ptr = shared_ptr;

		shared_ptr->getPoller()->doDelayTask(500, [this, weak_ptr]()->int
			{
				auto shared_ptr = weak_ptr.lock();
				if (!shared_ptr || !shared_ptr->alive())
					return 0;
				if (!shared_ptr->IsPlaying())
				{
	...
	{
		auto  shared_ptr = std::dynamic_pointer_cast<RtmpConnection>(shared_from_this());
		if (!shared_ptr)
			return;

		shared_ptr->Handshake();
		...
		std::weak_ptr<RtmpConnection>weak_ptr = shared_ptr;
		shared_ptr->getPoller()->doDelayTask(200, [weak_ptr, this]()->int
			{
				auto shared_ptr = weak_ptr.lock();
				if (!shared_ptr || !shared_ptr->alive())
					return 0;

				if (!shared_ptr->IsPublishing())
				{
					if (timestamp_.Elapsed() > 3000)
					{
						shared_ptr->shutdown();
				...
				auto pub_shared_ptr = rtmp_publisher_.lock();
				if (pub_shared_ptr)
				{
					pub_shared_ptr->video_timestamp_ = 0;
					pub_shared_ptr->audio_timestamp_ = 0;
					has_key_frame_ = true;
					if (pub_shared_ptr->media_info_.video_codec_id == RTMP_CODEC_ID_H264) {
...
bool RtmpConnection::HandleMessage(RtmpMessage& rtmp_msg)
{
    ...
    {        
        ...
        case RTMP_FLEX_MESSAGE:
			/*LOG_INFO*/PrintE("unsupported rtmp flex message.\n");
        ...
        case RTMP_FLASH_VIDEO:
			/*LOG_INFO*/PrintE("unsupported rtmp flash video.\n");
			ret = false;
            break;    
        case RTMP_ACK:
            break;            
        ...
        case RTMP_USER_EVENT:
            break;
        default:
			/*LOG_INFO*/PrintE("unkonw message type : %d\n", rtmp_msg.type_id);
            break;
    }
...
{
    /*LOG_INFO*/PrintE("[Publish] app: %s, stream name: %s, stream path: %s\n", app_.c_str(), stream_name_.c_str(), stream_path_.c_str());
...
{
	/*LOG_INFO*/PrintE("[Play] app: %s, stream name: %s, stream path: %s\n", app_.c_str(), stream_name_.c_str(), stream_path_.c_str());
...
bool RtmpConnection::SendMediaData(uint8_t type, uint64_t timestamp, std::shared_ptr<char> payload, uint32_t payload_size)
{
	...
	/*task_scheduler_->AddTriggerEvent*/conn->async([conn, type, timestamp, payload, payload_size] {
...
bool RtmpConnection::SendVideoData(uint64_t timestamp, std::shared_ptr<char> payload, uint32_t payload_size)
{
	...
	/*task_scheduler_->AddTriggerEvent*/conn->async([conn, timestamp, payload, payload_size] {
		RtmpMessage rtmp_msg;
		rtmp_msg.type_id = RTMP_VIDEO;
		rtmp_msg._timestamp = timestamp;
		rtmp_msg.stream_id = conn->stream_id_;
