...
const char* FindFirstCrlf(const std::string&str){
	auto crlf = std::search(str.cbegin(), str.cend(), kCRLF, kCRLF + 2);
	return crlf == str.cend() ? nullptr : &*crlf;
}

const char* FindLastCrlf(const std::string& str) {
	auto crlf = std::find_end(str.cbegin(), str.cend(), kCRLF, kCRLF + 2);
	return crlf == str.cend() ? nullptr : &*crlf;
}

const char* FindLastCrlfCrlf(const std::string& str){
	char crlfCrlf[] = "\r\n\r\n";
	auto crlf = std::find_end(str.cbegin(), str.cend(), crlfCrlf, crlfCrlf + 4);
	return crlf == str.cend() ? nullptr : &*crlf;
}

bool RtspRequest::ParseRequest(/*BufferReader *buffer*/std::string& buffer)
{
	if(/*buffer->Peek()[0]*/(!buffer.empty()) && buffer[0] == '$') {
		method_ = RTCP;
		return true;
	}
	...
	while(1) {
		if(state_ == kParseRequestLine) {
			const char* firstCrlf = /*buffer->*/FindFirstCrlf(buffer);
			if(firstCrlf != nullptr)
			{
				ret = ParseRequestLine(/*buffer->Peek()*/buffer.c_str(), firstCrlf);
				//buffer->RetrieveUntil(firstCrlf + 2);
				buffer = buffer.substr(firstCrlf - buffer.c_str() + 2);
			}
			...
			const char* lastCrlf = FindLastCrlf(buffer);//buffer->FindLastCrlf();
			if(lastCrlf != nullptr) {
				ret = ParseHeadersLine(/*buffer->Peek()*/buffer.c_str(), lastCrlf);
				//buffer->RetrieveUntil(lastCrlf + 2);
				buffer.substr(lastCrlf - buffer.c_str() + 2);
			}
			break;
		}
...
bool RtspResponse::ParseResponse(/*xop::BufferReader *buffer*/std::string& buffer)
{
	if (strstr(/*buffer->Peek()*/buffer.c_str(), "\r\n\r\n") != NULL) {
		if (strstr(/*buffer->Peek()*/buffer.c_str(), "OK") == NULL) {
			return false;
		}

		const char* ptr = strstr(/*buffer->Peek()*/buffer.c_str(), "Session");
		if (ptr != NULL) {
		...
		buffer = buffer.substr(strstr(/*buffer->Peek()*/buffer.c_str(), "\r\n\r\n") + 4 - buffer.c_str());
	}
