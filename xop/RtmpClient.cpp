...
RtmpClient::FrameCallback RtmpClient::FrameCB()
{
	std::lock_guard<std::mutex> lock(mutex_);
	return frame_cb_;
}
...
int RtmpClient::OpenUrl(std::string url, int msec, std::string& status)
{
	...
	rtmp_conn_->startConnect(ip_, port_);
	...
	//	xop::Timer::Sleep(100);
	//	timeout -= 100;
	//} while (rtmp_conn_->alive()/*!IsClosed()*/ && !rtmp_conn_->IsPlaying() && timeout > 0);
...
bool RtmpClient::IsConnected()
{
	...
	if (rtmp_conn_ != nullptr) {
		return rtmp_conn_->alive();//(!rtmp_conn_->IsClosed());
	}
	return false;
}
