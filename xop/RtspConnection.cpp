...
void RtspConnection::onRecv(const toolkit::Buffer::Ptr& pBuf)
{
	KeepAlive();

	rtsp_data_buffer_.append(pBuf->data(), pBuf->size());
	...
	if (conn_mode_ == RTSP_SERVER) {
		if (!HandleRtspRequest(rtsp_data_buffer_)){
			//return false; 
			return;
		}
	}
	else if (conn_mode_ == RTSP_PUSHER) {
		if (!HandleRtspResponse(rtsp_data_buffer_)) {
			//return false;
			return;
		}
	}
	...
	if (rtsp_data_buffer_.size() > MAX_RTSP_MESSAGE_SIZE)
		rtsp_data_buffer_.clear();
	//return true;
}

void xop::RtspConnection::onError(const toolkit::SockException& ex)
{
	WarnL << ex.what();
	OnClose();
}
...
void RtspConnection::OnClose()
{
	if(session_id_ != 0) {
		...
		if (rtsp) {
			...
			if (media_session) {
				media_session->RemoveClient(this->getSock()->rawFD()/*GetSocket()*/);
			}
		}
	}
...
bool RtspConnection::HandleRtspRequest(std::string& buffer/*BufferReader& buffer*/)
{
#if RTSP_DEBUG
...
bool RtspConnection::HandleRtspResponse(std::string& buffer/*BufferReader& buffer*/)
{
#if RTSP_DEBUG
	string str(buffer.Peek(), buffer.ReadableBytes());
	if (str.find("rtsp") != string::npos || str.find("RTSP") != string::npos) {
...
void RtspConnection::SendRtspMessage(std::shared_ptr<char> buf, uint32_t size)
{
	...
	this->send(buf.get(), size);
	return;
}

void RtspConnection::HandleRtcp(std::string& buffer/*BufferReader& buffer*/)
{   
	const char* peek = buffer.c_str();//buffer.Peek();
	if(peek[0] == '$' &&  /*buffer.ReadableBytes()*/buffer.size() > 4) {
		uint32_t pkt_size = peek[2]<<8 | peek[3];
		if(pkt_size +4 >= buffer.size()/*buffer.ReadableBytes()*/) {
			//buffer.Retrieve(pkt_size +4);  
			buffer = buffer.substr(pkt_size + 4);
		}
	}
}
...
void RtspConnection::HandleCmdDescribe()
{
	...
	if (rtp_conn_ == nullptr) {
		rtp_conn_.reset(new RtpConnection(std::dynamic_pointer_cast<toolkit::TcpClient>(shared_from_this())));
	}
	...
	if(!rtsp || !media_session) {
		...
		media_session->AddClient(/*this->GetSocket()*/getSock()->rawFD(), rtp_conn_);
		...
		std::string sdp = media_session->GetSdpMessage(/*SocketUtil::GetSocketIp(this->GetSocket())*/this->get_peer_ip(), rtsp->GetVersion());
		if(sdp == "") {
	...
	if(media_session->IsMulticast())  {
		...
		if(rtsp_request_->GetTransportMode() == RTP_OVER_TCP) {
			...
			if(rtp_conn_->SetupRtpOverUdp(channel_id, peer_rtp_port, peer_rtcp_port)) {
				auto rtcp_fd = rtp_conn_->GetRtcpSocket(channel_id);

				rtcp_fd->setOnRead([this](const toolkit::Buffer::Ptr& buf, struct sockaddr* addr, int size) {
					//接收到数据回调  [AUTO-TRANSLATED:1cd064ad]
					// Data received callback
					DebugL << "recv data form " << /*getIP(addr)*/get_peer_ip() << ":" << buf->data();
					//this->HandleRtcp(buf);
					});
...
void RtspConnection::SendAnnounce()
{
	...
	if (!rtsp || !media_session) {
		...
		session_id_ = media_session->GetMediaSessionId();
		media_session->AddClient(this->getSock()->rawFD()/*GetSocket()*/, rtp_conn_);
