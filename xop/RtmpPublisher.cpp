...
int RtmpPublisher::SetMediaInfo(const MediaInfo &media_info)
{
...
int RtmpPublisher::OpenUrl(const std::string &url, int msec, std::string& status)
{
	std::lock_guard<std::mutex> lock(mutex_);
	...
	if (this->ParseRtmpUrl(url) != 0) {
		/*LOG_INFO*/PrintE("[RtmpPublisher] rtmp url(%s) was illegal.\n", url.c_str());
		return -1;
	}

	/*LOG_INFO*/PrintI("[RtmpPublisher] ip:%s, port:%hu, stream path:%s\n", ip_.c_str(), port_, stream_path_.c_str());
	...
	rtmp_conn_->startConnect(ip_, port_, (float)timeout / 1000);
...
bool RtmpPublisher::IsConnected()
{
	...
	if (rtmp_conn_ != nullptr) {
		return rtmp_conn_->alive()/*!IsClosed())*/;
	}
	return false;
}
...
int RtmpPublisher::PushVideoFrame(uint8_t *data, uint32_t size)
{
	...
	if (rtmp_conn_ == nullptr || !rtmp_conn_->alive()/*IsClosed()*/ || size <= 5) {
		return -1;
	}
...
int RtmpPublisher::PushAudioFrame(uint8_t *data, uint32_t size)
{
	...
	if (rtmp_conn_ == nullptr || !rtmp_conn_->alive()/*IsClosed()*/ || size <= 0) {
		return -1;
	}
	...
	if (has_key_frame_ && media_info_.audio_codec_id == RTMP_CODEC_ID_AAC) {
		...
		//int XXX = data[0] << 8 | data[1];
		//int XXXX = data[0] | data[1]<<8;
